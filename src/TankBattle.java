 ...
 * Tank Battle
 *
 * <p>Debug: run Main.java in IDE (or run this class's main method directly). Package: use build.bat
 * / build.ps1 in project root to generate TankBattle.jar, then run with java -jar TankBattle.jar.
 *
 * <p>Controls: W A S D / Arrow keys : Move Space : Fire P : Pause / Resume R : Restart ESC : Quit
 */
public class TankBattle extends JPanel {
    ...
    class Tank {
        ...
        Rectangle rect() {
            return new Rectangle(x, y, TANK, TANK);
        }
    ...
    class Bullet {
        ...
        Rectangle rect() {
            return new Rectangle(x, y, BULLET, BULLET);
        }
    ...
    class Explosion {
        ...
        boolean done() {
            return System.currentTimeMillis() - born > 280;
        }
    }
    ...
    void buildMap() {
        for (int r = 0; r < ROWS; r++) for (int c = 0; c < COLS; c++) map[r][c] = Tile.EMPTY;

        // Outer steel walls
        for (int i = 0; i < COLS; i++) {
            map[0][i] = Tile.STEEL;
            map[ROWS - 1][i] = Tile.STEEL;
        }
        for (int i = 0; i < ROWS; i++) {
            map[i][0] = Tile.STEEL;
            map[i][COLS - 1] = Tile.STEEL;
        }
    ...
    boolean tryMove(Tank t) {
        ...
        if (nx < CELL
                || ny < CELL
                || nx + TANK > (COLS - 1) * CELL
                || ny + TANK > (ROWS - 1) * CELL) return false;
        // Walls
    ...
    void spawnEnemies() {
        long now = System.currentTimeMillis();
        if (enemiesOnField < maxOnField
                && enemiesRemaining > enemiesOnField
                && now - lastSpawn > 1500) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.drawString(
                paused ? "[PAUSED]" : (gameOver ? "[GAME OVER]" : (win ? "[WIN]" : "[PLAYING]")),
                340,
                hy);
    ...
    void drawTank(Graphics2D g2, Tank t) {
        ...
        switch (t.dir) {
            case UP:
                g2.fillRect(cx - 1, t.y - 4, 2, TANK / 2 + 4);
                break;
            case DOWN:
                g2.fillRect(cx - 1, cy, 2, TANK / 2 + 4);
                break;
            case LEFT:
                g2.fillRect(t.x - 4, cy - 1, TANK / 2 + 4, 2);
                break;
            case RIGHT:
                g2.fillRect(cx, cy - 1, TANK / 2 + 4, 2);
                break;
        }
    }
